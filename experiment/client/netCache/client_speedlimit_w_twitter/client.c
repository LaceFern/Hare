...
uint32_t rand_op_arr[1000];
uint32_t rand_fre_arr[1000];
...
// #define HC_US 50000000//200000000//50000000//50000000//5000000//50000//5000000//6000//1000
// #define HC_UPD_US 5000000//5000000//300000000//20000000
// #define SENDEND_US 85000000//70000000//250000000//5000000//50000//5000000//6000//1000
// #define END_US 90000000//75000000//270000000//200000000//5000000//50000//5000000//6000//1000
// #define HC_US 100000000
// #define HC_UPD_US 100000000
// #define SENDEND_US 300000000
// #define END_US 301000000
...
uint32_t wpkts_send_limit_ms = 5000;//2000;//8192;//8192;
...
struct_zipfPara gb_zipfPara={
    ...
    1,
    5000000,
    32768,//8192,//2048,//32768,//10000, //10000, //500,
    0,
    100,
    64,
    1000000,
...
static int32_t np_client_txrx_loop_kvs(uint32_t lcore_id, uint32_t client_id, uint16_t rx_queue_id, struct_zipfPara zipfPara, FILE* fout, FILE* fout_fpgaCheck) {
    ...
    char buffer[256]; // 用于存储每一行的内�??
    int line_count = 0;
    // 读取文件的每一行，跳过�?一行（标�?�）
    fgets(buffer, sizeof(buffer), file); // 跳过标�?��??
    ...
    uint32_t duplicate_times = 5000; //>=0  (500)
    uint32_t compress_ratio = 1; //>=1;
    uint32_t debug_end = 25567660;
    ...
    for(int i = 0; i < line_count; i++){
        if(rand_op_arr[i] > 50){
            frequency_arr[i] += 1.0 * rand_fre_arr[i] / 150;
        }
        else{
            frequency_arr[i] -= 1.0 * rand_fre_arr[i] / 150;
        }
    ...
    while (1) {
        ...
        // RX
        ////fprintf(fout, "613 check point: rx start.\n");
        int loop = 10 * MAX_CLIENT_IN_LCORE / NC_MAX_BURST_SIZE;
        if(loop == 0) loop = 1;
        for(int c = 0; c < loop; c++){ 
            for (int i = 0; i < lconf->n_rx_queue; i++) {
                uint32_t nb_rx = rte_eth_rx_burst(lconf->port, lconf->rx_queue_list[i], mbuf_burst, NC_MAX_BURST_SIZE);
        ...
        // TX: generate packet, put in TX queue
        ////fprintf(fout, "613 check point: tx start.\n");
        if (pkts_send_ms < wpkts_send_limit_ms * frequency_arr[second_count]) {
            for(int c = 0; c < MAX_CLIENT_IN_LCORE; c++){
                int recir_flag = 0;
                // stage:send batch--require
                if(sendEnd_flag[c] == 0){
...
int main(int argc, char **argv) {
    ...
    pthread_barrier_init(&trace_init, NULL, n_lcores - 1);
    ...
    // uint32_t *rand_op_arr = (uint32_t *)malloc(sizeof(uint32_t *) * 1u * 1000);
    // uint32_t *rand_fre_arr = (uint32_t *)malloc(sizeof(uint32_t *) * 1u * 1000);
    srand(0);
    for(int i = 0; i < 1000; i++){
        rand_op_arr[i] = rand() % 100;
        rand_fre_arr[i] = rand() % 100;
    }
