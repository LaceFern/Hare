...
int main(){
    double zipfPara_n_arr[] = {1000000}; //9000000 //key num in dataset
    double zipfPara_a_arr[] = {0.99};
    double zipfPara_reqNum_arr[] = {1000000}; // req num in test
    double zipfPara_lfsrSeed_arr[] = {1}; // req num in test
    int type_arr[] = {1};
    double changeNum_arr[] = {100};
            ...
            printf("info: binary research preperation ends.\n");
                ...
                for(unsigned int zipfPara_reqNum_idx = 0; zipfPara_reqNum_idx < zipfPara_reqNum_arr_len; zipfPara_reqNum_idx++){
                    ...
                    for(unsigned int zipfPara_lfsrSeed_idx = 0; zipfPara_lfsrSeed_idx < zipfPara_lfsrSeed_arr_len; zipfPara_lfsrSeed_idx++){
                        ...
                        // freqCheckRes_out << "f" << "\tP" << "\tnum" << endl;
                        // float f = 0;
                        // float P = 0;
                        // int num = 225280;//zipfPara_n;
                        // int k10_count = 0;
                        // for(unsigned int i = 0; i < num; i++){
                        //     f = (double)(freq_arr[i]) / 0xFFFFFFFF;
                        //     P += f;
                        //     k10_count++;
                        //     // if(k10_count == 1024 && i < zipfPara_n){
                        //     //     k10_count = 0;
                        //     //     freqCheckRes_out << f << "\t" << P << "\t" << (i + 1) << endl;
                        //     // }
                        //     if(i == 32768-1 || i == 65536-1 || i == 163840-1){
                        //         freqCheckRes_out << f << "\t" << P << "\t" << (i + 1) << endl;
                        //     }
                        // }
                        ...
                        ofstream freqCheckRes_out;
                        freqCheckRes_out.open(fn_freqCheckRes);
                        freqCheckRes_out << "test" << "\tcal" << endl;
                        for(unsigned int i = 0; i < zipfPara_n; i++){
                            freqCheckRes_out << ((double)(freqCheck_arr[i])) / zipfPara_reqNum << "\t" << (double)(freq_arr[i]) / 0xFFFFFFFF << endl;
                        }
                        freqCheckRes_out.close();
                        free(freqCheck_arr);
