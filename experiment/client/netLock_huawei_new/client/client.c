            ...
            for(int c = 0; c < MAX_CLIENT_IN_LCORE; c++){
                ...
                if(stepFlag[c] == 0 && sendEnd_flag[c] == 0){
                    ...
                    for(int i = 0; i < txnInfo.lock_num; i++){
                        ...
                        // }            
                        if(((int)(lockId_arr[lockIdxInTxn]) - hc_flag * 10000) < 0){ //虽然存在循环的问题但以目前的替换次数还不会循环
        ...
        if (pkts_send_ms < wpkts_send_limit_ms) {
            for(int c = 0; c < MAX_CLIENT_IN_LCORE; c++){
                ...
                if(sendEnd_flag[c] == 0){
                    ...
                    if(((int)(req_arr[reqIdx]) - hc_flag * 10000) < 0){ //虽然存在循环的问题但以目前的替换次数还不会循环
