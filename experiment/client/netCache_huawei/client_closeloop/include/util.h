...
struct rte_ether_addr mac_dst_hc_arr[HC_SLAVE_NUM] = {
    ...
    {.addr_bytes = {0x00, 0x02, 0x00, 0x00, 0x03, 0x00}}
};
...
uint64_t responseTime[800][800] = {0};
...
static void print_time_trace_kvs(uint64_t times, FILE* fout_stats) {
    ...
    for(int i = 0; i < times; i++){
        ...
        int p99_index = 400 * 0.99;

        uint64_t sum = 0;
        for (ii = 1; ii < 400; ii++){
            sum += responseTime[i][ii];
        }
        fprintf(fout_stats, "%0.2f\t%0.2f\n", 1.0 * responseTime[i][p99_index] / (rte_get_tsc_hz() / 1000000), 1.0 * sum / 400 / (rte_get_tsc_hz() / 1000000));
