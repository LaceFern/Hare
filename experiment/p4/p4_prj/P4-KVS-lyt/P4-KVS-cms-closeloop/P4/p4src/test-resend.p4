    ...
    inout ingress_intrinsic_metadata_for_tm_t       ig_tm_md)
{
    ...
    // L2 forward for congest control
    table mac_forward_4cc
    {
        key = {
    ...
    apply
    {
        ...
        if(hdr.kv.isValid())
        {
            ...
            if(index_mapping_table.apply().hit)
            {
                ...
                if(hdr.kv.op_code == OP_GET || hdr.kv.op_code == OP_GET_RESEND){
                    reflect();
            ...
            else
            {
                ...
                if(ig_intr_md.ingress_port == SAMPLE_PORT && clean_flag == 0 && hdr.kv.op_code != OP_GET_RESEND){
                // if(clean_flag == 0 && hdr.kv.op_code != OP_GET_RESEND){
                    bool hot = false;
                    SKETCH_FUNC(hot)
                    // bool toCP = false;
                    if(hot == true){
                ...
                mac_forward_4cc.apply();// mac_forward.apply();// hash_partition_forward.apply();
                // set_bridge(HEADER_TYPE_NORMAL);

                // // congestion
                // bool congested = false;
                // check_congestion(congested, ig_tm_md.ucast_egress_port);
                // if(congested && (hdr.kv.op_code == OP_GET || hdr.kv.op_code == OP_GET_RESEND))
                // {
                //     drop();
                //     // reflect();
                // }
        ...
        {
            if(hdr.ctrl.op_code == LOCK_HOT_QUEUE)
            {
                send(CPU_CONTROLLER);
            }
            else if(hdr.ctrl.op_code == OP_CALLBACK)
            {
                send(CONTROL_PLANE);
            }
            else
            {
                mac_forward.apply();//mac_forward.apply();// send(CONTROL_PLANE);
            }
        }
        else
        {
