...
static void nic_0_func(uint32_t lcore_id, P4Program &program) {
	...
	fin_hotspots.open("/home/zxy/p4_prj/P4-KVS-lyt/P4-KVS-cms-hitrate/P4/c2k_last_hotspots_2k.txt",ios::in);
		...
		for(int i = 0; i < rcv_num; i++){
			...
			if(lock_ctl_header->op == HOT_REPORT){
				...
				if(hot_obj_num == 0){
                    ...
                    /****************** r e c o r d - h o t s p o t s ******************/
                    if(hot_obj_num == 0){
			            total_report_num += TOPK;
			            if(hitrate_flag == 1) fout_hotspots << "dontknow" << "\t" << "dontknow" << "\t" << hit_rate_total << "\t" << 1 - true_offloaded_count/SLOT_SIZE << "\t" << valid_report_num / total_report_num << endl;
                    }
