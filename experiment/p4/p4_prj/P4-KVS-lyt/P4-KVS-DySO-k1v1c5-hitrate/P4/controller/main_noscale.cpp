...
static void nic_0_func(uint32_t lcore_id, P4Program &program) {
	...
	fin_hotspots.open("/home/zxy/p4_prj/P4-KVS-lyt/P4-KVS-DySO-k1v1c5-hitrate/P4/c32k_last_hotspots_32k.txt",ios::in);
	...
	while(true){
		...
		if(timeuse_ini > 30000000){
			...
			if(quit_flag == 0){
				...
				cout << "Number of offload objects = " << offload_total;
				cout << "Time (us) to offload objects = " << timeuse_offload;
				cout << "Average time (us) to offload objects = " << timeuse_offload / offload_total;
				fout_hotspots.close();
				for(uint32_t row_idx = 0; row_idx < KEY_ROWNUM; row_idx++){
					for(uint32_t col_idx = 0; col_idx < KEY_COLNUM; col_idx++){
