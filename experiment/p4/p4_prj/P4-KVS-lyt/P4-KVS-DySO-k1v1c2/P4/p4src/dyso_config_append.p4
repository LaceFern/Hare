...
#define KEY_COLNUM 2 // the column of the r-mat
...
#define KEY_REGLENLENWIDTH 10 // 12b(4096)-2b
...
#define KEY_REG(regIndex, segIndex, ctrlIndex) \
    ...
    RegisterAction<bit<KEY_REGWIDTH>, _, bit<KEY_REGWIDTH>>(key_##regIndex) key_##regIndex##_update = {\
        void apply(inout bit<KEY_REGWIDTH> val){\
            val = hdr.ctrl.key_##segIndex##_##ctrlIndex;\
        }\
    };
...
// //-----------------------------------------------------------------
// // 4 show
//     table col1_objSeg0_operation_mat{
//         key = {
//             meta.col0_objSeg0_hit : ternary;
//             meta.col0_objSeg1_hit : ternary;
//             meta.col0_objSeg2_hit : ternary;
//             meta.col0_objSeg3_hit : ternary;
//         }
//         actions = {
//             col1_objSeg0_read;
//             NoAction;
...
#define KEY_COLS_ACTIONandMAT()\
    ...
    KEY_LASTCOLHIT_ACTIONandMAT(0, 0, 1, 2, 3)\
    KEY_LASTCOLHIT_ACTIONandMAT(1, 4, 5, 6, 7)
...
#define KEY_COLS_APPLY()\
    ...
    key_lastColHitAction_mat_1.apply();
...
#define HASH_KEY2INDEX_APPLY()\
    ...
    if(hdr.kv.isValid()){\
        ...
        meta.kv.hash_index = hdr.ctrl.hash_index;\
    }
...
#define VALUE_COLNUM 2 // the column of the r-mat (must equal to VALUE_COLNUM)
...
RegisterAction<bit<VALUE_REGWIDTH>, _, bit<VALUE_REGWIDTH>>(value_##regIndex) value_##regIndex##_update = {\
    void apply(inout bit<VALUE_REGWIDTH> val){\
        val = hdr.ctrl.value_##segIndex##_##ctrlIndex;\
    }\
};
...
#define VALUE_MODIFY_COL_RUN(colIndex, regIndex0, regIndex1, regIndex2, regIndex3)\
    value_##regIndex0##_update.execute(hdr.ctrl.hash_index);\
    value_##regIndex1##_update.execute(hdr.ctrl.hash_index);\
    value_##regIndex2##_update.execute(hdr.ctrl.hash_index);\
    value_##regIndex3##_update.execute(hdr.ctrl.hash_index);
