...
//-----------------------------------------------
#define TS_TYPE_HC 0 // hotspot changes
#define TS_TYPE_CLEANEND 1
#define TS_TYPE_PACKET100 100
#define TS_TYPE_PACKET200 200
#define TS_TYPE_PACKET300 300
#define TS_TYPE_PACKET400 400
#define TS_TYPE_PACKET500 500
#define TS_TYPE_PACKET600 600
#define TS_TYPE_PACKETEND 7
#define TS_TYPE_PROCESSEND 8
#define INTERVAL_US 30000000 
typedef struct struct_timestamp{
	long timestamp;
	uint32_t type;
	uint32_t tag;
}st_ts;
st_ts timestamp_0[10000];
st_ts timestamp_1[10000];

st_ts tmp_timestamp_0;
st_ts tmp_timestamp_1;
uint32_t timestamp_count_0 = 0;
uint32_t timestamp_count_1 = 0;
uint32_t packet_count = 0;
uint32_t cleanend_flag = 0;
uint32_t needreport_flag = 0; // no use
uint32_t givereport_flag = 0; // no use
uint32_t timestamp_start_flag = 0;
long timestamp_start;
long timestamp_current;
long timestamp_end; // no use
//-----------------------------------------------
...
[[noreturn]] void hotin_thread(){
	cout << "Initiating hotin watching..." << endl;
	// init network
	int sock_fd;
	sockaddr_ll sl{};
	{
	...
	if (len < 0) {
		...
		tmp_timestamp_0.timestamp = 1000000 * tmp_timeval.tv_sec + tmp_timeval.tv_usec;
		tmp_timestamp_0.type = TS_TYPE_HC;
		tmp_timestamp_0.tag = 0;
		timestamp_0[timestamp_count_0] = tmp_timestamp_0;
		timestamp_count_0++;
		timestamp_start = 1000000 * tmp_timeval.tv_sec + tmp_timeval.tv_usec;
		printf("timestamp starts!\n");
		timestamp_start_flag = 1;
	}
	if(timestamp_start_flag == 1){
		struct timeval tmp_timeval;
		gettimeofday(&tmp_timeval, NULL );
		timestamp_current = 1000000 * tmp_timeval.tv_sec + tmp_timeval.tv_usec;
		if(timestamp_current - timestamp_start > INTERVAL_US){
			timestamp_start_flag = 0;
			//analyze timeline
			printf("timestamp ends!\n");
		}
	}
}
...
[[noreturn]] void control_thread(uint32_t &lock_id, bool &ready, mutex &mutex, condition_variable &cv)
{
	...
	while(true){
		...
		if(timestamp_start_flag == 1){
			...
			struct timeval tmp_timeval;
			gettimeofday(&tmp_timeval, NULL );
			tmp_timestamp_1.timestamp = 1000000 * tmp_timeval.tv_sec + tmp_timeval.tv_usec;
			if(packet_count == 100 || packet_count == 200 || packet_count == 300 || 
				packet_count == 400 || packet_count == 500 || packet_count == 600){
				tmp_timestamp_1.type = packet_count;
				tmp_timestamp_1.tag = 0;
				timestamp_1[timestamp_count_1] = tmp_timestamp_1;
				timestamp_count_1++;
				printf("packet_count = %d, time = %d us\n", packet_count, tmp_timestamp_1.timestamp - timestamp_start);
			}
			else{
				tmp_timestamp_1.type = TS_TYPE_PACKETEND;
				tmp_timestamp_1.tag = 0;
			}
		}
		//end--------------------------------------------------
		...
		if(lock_header->op_type == OP_GET){
			if(ready == false && (lock.owns_lock() || lock.try_lock())){
...
int main(int argc, char **argv)
{
	...
	while(true){
		...
		// if(t - ts > 5000000)
		if(t_minus_ts > 1000000) //300000//1000000//5000000
		{		
			...
			if(timestamp_start_flag == 1){
				...
				timestamp_0[timestamp_count_0] = tmp_timestamp_0;
				timestamp_count_0++;
				printf("process ends, time = %d us\n", tmp_timestamp_0.timestamp - timestamp_start);
				// clean end
				cleanend_flag = 1;
				struct timeval tmp_timeval;
				gettimeofday(&tmp_timeval, NULL );
				tmp_timestamp_0.timestamp = 1000000 * tmp_timeval.tv_sec + tmp_timeval.tv_usec;
				tmp_timestamp_0.type = TS_TYPE_CLEANEND;
				tmp_timestamp_0.tag = 0;
				timestamp_0[timestamp_count_0] = tmp_timestamp_0;
				timestamp_count_0++;
				printf("clean ends, time = %d us\n", tmp_timestamp_0.timestamp - timestamp_start);
...
//			cout << "Cannot replace id " << id << endl;
		...
		if(timestamp_start_flag == 1){
			...
			gettimeofday(&tmp_timeval, NULL );
			tmp_timestamp_0.timestamp = 1000000 * tmp_timeval.tv_sec + tmp_timeval.tv_usec;
			tmp_timestamp_0.type = TS_TYPE_PROCESSEND;
			tmp_timestamp_0.tag = report_count;
		}
		//end--------------------------------------------------
	}
