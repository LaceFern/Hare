...
static void nic_0_func(uint32_t lcore_id, P4Program &program) {
	...
	fin_hotspots.open("/home/zxy/p4_prj/P4-KVS-lyt/P4DPDK-KVS-noturn/P4/no.txt",ios::in);//c32k_last_hotspots_32k
	...
	while(true){
		...
		if(timeuse_ini > 50000000){
			...
			cout << "No packet comes within 50s, input 0 to wait, input others to quit:\n";
			cin >> quit_flag;
			if(quit_flag == 0){
				gettimeofday(&quit_start_ini, NULL );
			}
...
//				    }
			...
			// printf("211:TOPK(gettimeofday) %f us\n", timeuse_ini * 1.0);
		}
