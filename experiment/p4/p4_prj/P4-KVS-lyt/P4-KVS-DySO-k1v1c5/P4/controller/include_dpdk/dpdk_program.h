...
static uint32_t rcv_pkt_seq(uint32_t lcore_id, char *pkt, uint32_t pkt_size){
    ...
    // while(1){
        for (int i = 0; i < lconf->n_rx_queue; i++) {
            ...
            for (int j = 0; j < nb_rx; j++) {
                ...
                // //test
                // ETH_HEADER *eth = rte_pktmbuf_mtod(mbuf_rcv, ETH_HEADER *);
                // LOCK_CTL_HEADER *lock_ctl_header = (LOCK_CTL_HEADER*) ((uint8_t*)eth + 14);
                // printf("(%d, %d)\n", ntohl(lock_ctl_header->id[j]), ntohl(lock_ctl_header->index[j]));

                char* pkt_mem_rcv = rte_pktmbuf_mtod(mbuf_rcv, char *);
                rte_memcpy(pkt + rcv_count * pkt_size, pkt_mem_rcv, pkt_size);
...
struct rte_flow *generate_udp_flow(uint16_t port_phys, uint16_t port_udp, uint16_t rx_q, struct rte_flow_error *error){
    ...
    udp_spec.hdr.src_port = htons(port_udp); 
    udp_mask.hdr.src_port = 0x0003; //------------------------------------------------!!!!!!!!!!!!!!!!!
...
static int nc_init(int argc, char **argv) {
    ...
    for (portid_offset = 0; portid_offset < n_enabled_ports; portid_offset++) {
        ...
        //     flow = generate_udp_flow(portid, i, i / NC_RX_QUEUE_PER_LCORE, &error);
