...
//-------------------------------------------------
//-----------------------user begin---------------------
#define IP_LOCAL                "10.0.0.2"
#define MAC_LOCAL               {0x98, 0x03, 0x9b, 0xca, 0x48, 0x38} //amax 5
#define CLIENT_PORT             1111
#define HC_MASTER               0
#define HC_SLAVE_NUM            2//4//2
...
struct throughput_statistics {
    ...
    uint64_t last_req_timeout          ;
    uint64_t last_tx_get_re4wait        ;
...
#define SAMPLE_PER_CHECK 8000
uint64_t responseTime[800][8000] = {0};
uint64_t responseTime_upd_tsc = 0;
uint64_t responseTime_upd_times = 0;
int responseTime_count = 0;
...
static void print_per_core_throughput_kvs(void) {
    ...
    for (j = 0; j < n_lcores; j++) {
        ...
        //     "\tdropped_pkt\t%0.2f\n",
        //     i,
        //     1.0 * (tput_stat[i].tx                         -   tput_stat[i].last_tx            ),
        //     1.0 * (tput_stat[i].tx_get                     -   tput_stat[i].last_tx_get        ),
        //     1.0 * (tput_stat[i].tx_put                     -   tput_stat[i].last_tx_put        ),
        //     1.0 * (tput_stat[i].tx_del                     -   tput_stat[i].last_tx_del        ),
        //     1.0 * (tput_stat[i].rx                         -   tput_stat[i].last_rx            ),
        //     1.0 * (tput_stat[i].rx_getSucc                 -   tput_stat[i].last_rx_getSucc    ),
        //     1.0 * (tput_stat[i].rx_putSucc                 -   tput_stat[i].last_rx_putSucc    ),
        //     1.0 * (tput_stat[i].rx_delSucc                 -   tput_stat[i].last_rx_delSucc    ),
        //     1.0 * (tput_stat[i].rx_getFail                 -   tput_stat[i].last_rx_getFail    ),
        //     1.0 * (tput_stat[i].rx_putFail                 -   tput_stat[i].last_rx_putFail    ),
        //     1.0 * (tput_stat[i].rx_delFail                 -   tput_stat[i].last_rx_delFail    ),
        //     1.0 * (tput_stat[i].req_timeout                -   tput_stat[i].last_req_timeout   ),
        //     1.0 * (tput_stat[i].tx_get_re4wait             -   tput_stat[i].last_tx_get_re4wait),
        //     1.0 * (tput_stat[i].dropped_pkt                -   tput_stat[i].last_dropped_pkt   )
        // );
        ...
        dropped_pkt             +=  tput_stat[i].dropped_pkt                -   tput_stat[i].last_dropped_pkt           ;
        req_timeout             +=  tput_stat[i].req_timeout                -   tput_stat[i].last_req_timeout        ;
        tx_get_re4wait          +=  tput_stat[i].tx_get_re4wait             -   tput_stat[i].last_tx_get_re4wait        ;
        ...
        tput_stat[i].last_dropped_pkt             =   tput_stat[i].dropped_pkt           ;
        tput_stat[i].last_req_timeout                =   tput_stat[i].req_timeout        ;
        tput_stat[i].last_tx_get_re4wait             =   tput_stat[i].tx_get_re4wait           ;
    ...
    tput_stat_time[seconds].dropped_pkt                 = dropped_pkt              ; 
    tput_stat_time[seconds].req_timeout                 = req_timeout              ; 
    tput_stat_time[seconds].tx_get_re4wait                 = tx_get_re4wait              ;     
    seconds++;
...
static void print_time_trace_kvs(uint64_t times, FILE* fout_stats) {
    ...
    for(int i = 0; i < times; i++){
        ...
        (float)(tput_stat_time[i].dropped_pkt            ),
        (float)(tput_stat_time[i].req_timeout            ),
        (float)(tput_stat_time[i].tx_get_re4wait            )
        );

        // analyze response time
	    int ii, jj;
        uint64_t temp;
	    for (ii = 1; ii < SAMPLE_PER_CHECK; ii++){
	    	for (jj = 0; jj < SAMPLE_PER_CHECK - ii; jj++){
	    		if (responseTime[i][jj] > responseTime[i][jj + 1]){
	    			temp = responseTime[i][jj];
	    			responseTime[i][jj] = responseTime[i][jj + 1];
	    			responseTime[i][jj + 1] = temp;
	    		}
	    	}
	    }
        int p99_index = SAMPLE_PER_CHECK * 0.99;

        uint64_t sum = 0;
        for (ii = 1; ii < SAMPLE_PER_CHECK; ii++){
            sum += responseTime[i][ii];
        }
        fprintf(fout_stats, "%0.2f\t%0.2f\n", 1.0 * responseTime[i][p99_index] / (rte_get_tsc_hz() / 1000000), 1.0 * sum / SAMPLE_PER_CHECK / (rte_get_tsc_hz() / 1000000));
