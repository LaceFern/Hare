 ...
 * constants
 */ 
#define EXTENSIVE_OBJS_NORMAL_BENCHMARK 20
#define OBJ_PER_OBJ_NORMAL_BENCHMARK 150
#define EXTENSIVE_OBJS_MICRO_BENCHMARK_SHARED 55000
#define OBJ_PER_OBJ_MICRO_BENCHMARK_SHARED 1
#define EXTENSIVE_OBJS_MICRO_BENCHMARK_EXCLUSIVE 55000
#define OBJ_PER_OBJ_MICRO_BENCHMARK_EXCLUSIVE 1
#define EXTENSIVE_OBJS_TPCC 7000000
#define OBJ_PER_OBJ_TPCC 1
...
char deadobjed[MAX_CLIENT_NUM][MAX_TXN_NUM] = {0};
...
uint32_t num_ex[MAX_OBJ_NUM] = {0};
uint32_t num_sh[MAX_OBJ_NUM] = {0};
char busy[MAX_CLIENT_NUM][MAX_OBJ_NUM] = {0};
uint32_t ip_dst_pton, ip_src_pton;
int occupying_flag[MAX_OBJ_NUM] = {0};
...
int secondary_objs[MAX_OBJ_NUM];
...
uint32_t *(obj_type[MAX_CLIENT_NUM]);
uint32_t len[MAX_CLIENT_NUM];

//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
#define obj_NOTFIT 0x0 //must be larger than the range
#define MAX_OBJ_IN_SWITCH 32768//16384//16384//16382  MAX_OBJ_IN_SWITCH must larger than MAX_OBJ_IN_TXN
#define MAX_OBJ_IN_TXN 8//1//8!!!!!!!!!!!!!!!!!!!!!!!!!!!
#define MAX_CLIENT_IN_LCORE 512//512//1//512//1024//512//512//64
...
// #define DEADOBJ_CHECK_INTERVAL_US 1000//1000//6000//1000
#define RESEND_CHECK_PADDING_US 1000
#define HC_US 50000000//50000000//5000000//50000//5000000//6000//1000
#define SENDEND_US 250000000//5000000//50000//5000000//6000//1000
#define END_US 270000000//200000000//5000000//50000//5000000//6000//1000

#define CHECK_INTERVAL_US 1000000//1000000//100000
#define CHECK_START_US 0//0//45000000
#define CHECK_END_US 200000000//200000000//65000000 
...
// #define MAX_OBJ_IN_SWITCH_VALID 4096 //need to match obj_MASK
#define obj_MASK 0x7fff//0x7fff//0x3ffffff
#define obj_BIAS 0
#define obj_CAT  0x0
...
// #define ACQUIRE_OBJ            0x01
// #define RELEASE_OBJ            0x02
// #define RE_ACQUIRE_OBJ            0x03
// #define RE_RELEASE_OBJ            0x04

// op code
#define GRANT_OBJ 0x01
#define RELEA_OBJ 0x02
#define RE_GRANT_OBJ 0x81
#define RE_RELEA_OBJ 0x82

#define OBJ_GRANT_SUCC 0x03
#define OBJ_GRANT_FAIL 0x04
#define OBJ_RELEA_SUCC 0x05
#define OBJ_RELEA_FAIL 0x06
#define RE_OBJ_GRANT_SUCC 0x83
#define RE_OBJ_GRANT_FAIL 0x84
#define RE_OBJ_RELEA_SUCC 0x85
#define RE_OBJ_RELEA_FAIL 0x86

#define OBJ_QUEUE_SUCC 0x07
#define RE_OBJ_QUEUE_SUCC 0x87
//
// #define OBJ_GRANT_MODFAIL 0x08
// #define OBJ_RELEA_MODFAIL 0x09

#define OBJ_GRANT_MODFAIL_NFS 0x12
#define OBJ_GRANT_MODFAIL_HIT 0x13
#define OBJ_RELEA_MODFAIL_NFS 0x14
#define OBJ_RELEA_MODFAIL_HIT 0x15
//
#define OBJ_GRANT_BW_FAILED 0x16
#define OBJ_RELEA_BW_FAILED 0x17
...
#define obj_OVERFLOW  0xffffffff
...
typedef struct mine_zipfPara{
    double a;
    uint32_t n;
    uint32_t objNumInTxn;
    uint32_t traceNum;
    uint32_t traceNumReadIn;
    uint32_t lfsrSeed;
    uint64_t deadobj_check_interval_us;
...
static void generate_write_request_pkt(uint16_t rx_queue_id, uint8_t header_template[HEADER_TEMPLATE_SIZE], uint32_t lcore_id, struct rte_mbuf *mbuf, \
            struct_objInfo objInfo, struct_traceInfo txnInfo, uint32_t ip_src_addr, uint32_t ip_dst_addr, uint64_t timestamp) {
    ...
    // DEBUG!!
    uint16_t temp_port = 5002 + objInfo.obj_id % 23;
    // uint16_t temp_port = 5002;
    if(temp_port < 5002 || temp_port > 5024){
        printf("ERROR!!: dst_port = %d\n", temp_port);
    }
    if(objInfo.obj_id < 0 || objInfo.obj_id > 10009999){
        printf("ERROR!!: obj = %d\n", objInfo.obj_id);
    }
    udp->dst_port = htons(temp_port);
    //udp->dst_port = htons(5002 + objInfo.obj_id % 23); //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    
    MessageHeader* message_header = (MessageHeader*) ((uint8_t*)eth + HEADER_TEMPLATE_SIZE);
    message_header->padding = 0;
    message_header->opType = objInfo.action_type;
    message_header->obj = htonl(objInfo.obj_id);
    message_header->objMode = objInfo.obj_type;
    message_header->traceID = htonl(txnInfo.txn_id);
...
static int32_t np_client_txrx_loop(uint32_t lcore_id, uint32_t client_id, uint16_t rx_queue_id, struct_zipfPara zipfPara, FILE* fout, FILE* fout_fpgaCheck) {
    ...
    uint32_t *obj_arr = (uint32_t *)malloc(sizeof(uint32_t *) * zipfPara.traceNumReadIn * zipfPara.objNumInTxn);
    uint32_t *obj_hc_arr = (uint32_t *)malloc(sizeof(uint32_t *) * zipfPara.traceNumReadIn * zipfPara.objNumInTxn);
    char objFile[128];
    char objFile_hc[128];
    if(zipfPara.distribution_type == 0){
        sprintf(objFile, "./trace/res_zipf/a%0.2f_n%u_tN%d_sd%u.txt", zipfPara.a, zipfPara.n, zipfPara.traceNum, zipfPara.lfsrSeed);
        sprintf(objFile_hc, "./trace/res_zipf_hc/a%0.2f_n%u_tN%d_sd%u_hot-in_%u.txt", zipfPara.a, zipfPara.n, zipfPara.traceNum, zipfPara.lfsrSeed, zipfPara.hotChangedNum);    
    }
    else{
        sprintf(objFile, "./trace/res_rect/a%0.2f_n%u_tN%d_sd%u.txt", zipfPara.a, zipfPara.n, zipfPara.traceNum, zipfPara.lfsrSeed);
        sprintf(objFile_hc, "./trace/res_rect_hc/a%0.2f_n%u_tN%d_sd%u_hot-in_%u.txt", zipfPara.a, zipfPara.n, zipfPara.traceNum, zipfPara.lfsrSeed, zipfPara.hotChangedNum);    
    }
 

    printf("hotChangedNum: %u\n", zipfPara.hotChangedNum);
    printf("traceFile: %s\n", objFile);
    FILE *objFp=fopen(objFile,"r");
    FILE *objFp_hc=fopen(objFile_hc,"r");
    if(!objFp){
    ...
    for(uint32_t i = 0; i < zipfPara.traceNumReadIn; i++){
        // printf("check point 1.5: %d\n", i);
        fscanf(objFp,"%u", obj_arr+i);
        fscanf(objFp_hc,"%u", obj_hc_arr+i);
        if(i == 0) printf("idx=0, obj-d = %d, obj-u = %u\n", obj_arr[i], obj_arr[i]);
    }
    uint32_t traceIdx = 0;
    uint32_t objIdxInTrace = 0; 
    // printf("check point 2\n");
    //current txn info
    struct_objInfo ** objInTrace_arr = (struct_objInfo **)malloc(sizeof(struct_objInfo *) * MAX_CLIENT_IN_LCORE);
    int ** getRespFlag_arr = (int **)malloc(sizeof(int *) * MAX_CLIENT_IN_LCORE);
    for(int i = 0; i < MAX_CLIENT_IN_LCORE; i++){
        objInTrace_arr[i] = (struct_objInfo *)malloc(sizeof(struct_objInfo) * MAX_OBJ_IN_TXN);
        getRespFlag_arr[i] = (int *)malloc(sizeof(int) * MAX_OBJ_IN_TXN); 
    }
    // printf("check point 3\n");
    //---------------------------------------------------
    struct_traceInfo trace_arr[MAX_CLIENT_IN_LCORE];
    int stepFlag[MAX_CLIENT_IN_LCORE] = {0};
    uint32_t cid[MAX_CLIENT_IN_LCORE];
    uint32_t obj[MAX_CLIENT_IN_LCORE];
    for(uint32_t i = 0; i < MAX_CLIENT_IN_LCORE; i++){
        cid[i] = (client_id - 1) * MAX_CLIENT_IN_LCORE + i + CLIENT_ID_BIAS;
        obj[i] = cid[i];//rand();//cid[i];
    }
    for(int i = 0; i < MAX_CLIENT_IN_LCORE; i++){
        for(int j = 0; j < MAX_OBJ_IN_TXN; j++){
    ...
    //     // printf("pkts_send_ms = %d\n", pkts_send_ms);
        ...
        // RX
        int loop = MAX_CLIENT_IN_LCORE * zipfPara.objNumInTxn / NC_MAX_BURST_SIZE;
        if(loop == 0) loop = 1;
        for(int c = 0; c < loop; c++){ 
            for (int i = 0; i < lconf->n_rx_queue; i++) {
                ...
                for (int j = 0; j < nb_rx; j++) {
                    ...
                    uint32_t rcv_obj = ntohl(message_header->obj); 
        ...
        if (pkts_send_ms < wpkts_send_limit_ms) {
            for(int c = 0; c < MAX_CLIENT_IN_LCORE; c++){
                ...
                if(stepFlag[c] == 0 && sendEnd_flag[c] == 0){
                    ...
                    struct_traceInfo txnInfo;
                    txnInfo.obj_num = zipfPara.objNumInTxn;//MAX_OBJ_IN_TXN;//rand() % MAX_OBJ_IN_TXN + 1;
                    txnInfo.txn_id = cid[c];// rand();
                    txnInfo.cid = cid[c];
                    trace_arr[c] = txnInfo;

                    objIdxInTrace = traceIdx * zipfPara.objNumInTxn;
                    if(traceIdx == zipfPara.traceNumReadIn - 1){
                        traceIdx = 0;
                    } 
                    else{
                        traceIdx++;
                    }
                    for(int i = 0; i < txnInfo.obj_num; i++){
                        struct_objInfo objInfo;  

                        // objInfo.obj_id = obj[c]; 
                        // obj[c] ^= obj[c] >> 7;
                        // obj[c] ^= obj[c] << 20;
                        // obj[c] ^= obj[c] >> 13;
                        // objInfo.obj_id &= obj_MASK; //0x7fff;
                        //objInfo.obj_id = 0;
                        // objInfo.obj_id <<= obj_BIAS;
                        // objInfo.obj_id += obj_CAT;

                        if(hc_flag == 1){
                            objInfo.obj_id = obj_hc_arr[objIdxInTrace];
                        } 
                        else{
                            objInfo.obj_id = obj_arr[objIdxInTrace];
                        }                        
                        //objInfo.obj_id &= obj_MASK; //0x7fff;
                        objInfo.action_type = GRANT_OBJ;

                        // uint16_t randop_tmp = randop[c];
                        // if((randop_tmp & 0x1) == 1){
                        //     objInfo.obj_type = EXCLU_OBJ;
                        // }
                        // else{
                        //     objInfo.obj_type = SHARE_OBJ;
                        // }
                        objInfo.obj_type = SHARE_OBJ;//SHARE_OBJ;//SHARE_OBJ;//SHARE_OBJ;//EXCLU_OBJ;
                        randop[c] ^= randop[c] >> 7;
                        randop[c] ^= randop[c] << 9;
                        randop[c] ^= randop[c] >> 13;
                        
                        
                        objInfo.resend_count = 0;
                        objInfo.last_send_time = cur_tsc;
                        objInTrace_arr[c][i] = objInfo;
                        //fprintf(fout, "613 step 0: send obj_id:%d, txn_id:%d, cid:%d\n", objInfo.obj_id, txnInfo.txn_id, txnInfo.cid);
 
                        getRespFlag_arr[c][i] = RESPFLAG_ACQUIRE_WAIT;
                        mbuf = rte_pktmbuf_alloc(pktmbuf_pool[rx_queue_id]); 
                        generate_write_request_pkt(rx_queue_id, header_template_local, lcore_id, mbuf, objInfo, txnInfo, ip_src_pton, ip_dst_pton, 0);
                        enqueue_pkt_with_thres(lcore_id, mbuf, 32, 0); // enqueue_pkt_with_thres(lcore_id, mbuf, txnInfo.obj_num, 0);
                        pkts_send_ms ++; 
                        pkts_send_counter ++;
                        // printf("wait input:");
                        // getchar();
...
static int32_t np_client_loop(__attribute__((unused)) void *arg) {
    ...
    if ((lcore_id < 1)) {
        ...
        np_client_txrx_loop(lcore_id, lcore_id, lcore_id_2_rx_queue_id[lcore_id], gb_zipfPara, fout, fout_fpgaCheck);
        // gb_zipfPara.lfsrSeed++;
        printf("gb_zipfPara.lfsrSeed = %u\n", gb_zipfPara.lfsrSeed);
    }
    return 0; 
}
...
static void nc_parse_args_help(void) {
    printf("simple_socket [EAL options] --\n"
           ...
           "  -t number of req in a trace file\n"
           "  -s lfsr seed\n"
...
static int nc_parse_args(int argc, char **argv) {
    int opt;
    while ((opt = getopt(argc, argv, "n:a:t:s:h:z:w:e:")) != -1) {
        switch (opt) {
        case 'n':
        ...
        case 'a':
            ...
            break; 
        case 't':
            gb_zipfPara.traceNum = atoi(optarg);
            gb_zipfPara.traceNumReadIn = gb_zipfPara.traceNum;
            break;
        case 's':
        ...
        case 'h':
            ...
            printf("gb_zipfPara.hotChangedNum = %d", gb_zipfPara.hotChangedNum);
            break;
        case 'z':
...
int main(int argc, char **argv) {
    ...
    char logFile[128];
    sprintf(logFile, "./log/n%u_a%0.2f_t%u_s%u_h%u_z%u_w%u_e%u.txt", \
    gb_zipfPara.n, gb_zipfPara.a, gb_zipfPara.traceNum, \
    gb_zipfPara.lfsrSeed, \
    gb_zipfPara.hotChangedNum, gb_zipfPara.distribution_type, gb_zipfPara.fpga_wait_ms, gb_zipfPara.fpga_cacheSetNum);
    fout = fopen(logFile, "w");

    char statsFile[128];
    sprintf(statsFile, "./stats/n%u_a%0.2f_t%u_s%u_h%u_z%u_w%u_e%u.txt", \
    gb_zipfPara.n, gb_zipfPara.a, gb_zipfPara.traceNum, \
    gb_zipfPara.lfsrSeed, \
    gb_zipfPara.hotChangedNum, gb_zipfPara.distribution_type, gb_zipfPara.fpga_wait_ms, gb_zipfPara.fpga_cacheSetNum);
    ...
    //print statistics
    print_time_trace((CHECK_END_US - CHECK_START_US) / CHECK_INTERVAL_US, fout_stats);
