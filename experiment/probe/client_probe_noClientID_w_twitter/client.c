...
#define MAX_LOCK_IN_TXN 1//1//8!!!!!!!!!!!!!!!!!!!!!!!!!!!
#define MAX_CLIENT_IN_LCORE 1//512//1//512//1024//512//512//64
#define RESEND_CHECK_INTERVAL_US 20//2000//200//50// 5000000 // consider adjusting it dynamically, fpga drops packets!!!!!
// #define RESEND_MAX_COUNT 100
// #define DEADLOCK_CHECK_INTERVAL_US 1000//1000//6000//1000
#define RESEND_CHECK_PADDING_US 0
// #define HC_US 100000000//200000000//50000000//50000000//5000000//50000//5000000//6000//1000
// #define HC_UPD_US 300000000//5000000//30000000//20000000
// #define SENDEND_US 402000000//70000000//250000000//5000000//50000//5000000//6000//1000
// #define END_US 403000000//75000000//270000000//200000000//5000000//50000//5000000//6000//1000
// #define HC_US 20000000
// #define HC_UPD_US 5000000
// #define SENDEND_US 81000000
// #define END_US 82000000

#define HC_US 40000000
#define HC_UPD_US 40000000
#define SENDEND_US 160000000
#define END_US 161000000

// #define CHECK_INTERVAL_US 50000//100000//1000000//100000
// #define CHECK_START_US 48000000//45000000//0//45000000
// #define CHECK_END_US 65000000//85000000//65000000//200000000//65000000 

#define CHECK_INTERVAL_US 1000000//100000//1000000//100000
#define CHECK_START_US 0//48000000//45000000//0//45000000
#define CHECK_END_US 160000000//65000000//85000000//65000000//200000000//65000000 
...
#define LOCKID_MASK 0x7fff//0x7fff//0x3ffffff
...
uint32_t wpkts_send_limit_ms = 8192;//8192;//100;//8192;//8192;
...
static void generate_lock_request_pkt(uint16_t rx_queue_id, uint8_t header_template[HEADER_TEMPLATE_SIZE], uint32_t lcore_id, struct rte_mbuf *mbuf, \
    ...
    // DEBUG!!
    uint16_t temp_port = 5002 + lockInfo.lock_id % 23;
...
static void generate_kv_request_pkt(uint16_t rx_queue_id, uint8_t header_template[HEADER_TEMPLATE_SIZE], uint32_t lcore_id, struct rte_mbuf *mbuf, \
    ...
    // DEBUG!!
    uint16_t temp_port = 5002 + reqInfo.key % 23;//uint16_t temp_port = 5002 + reqInfo.key % 47;//uint16_t temp_port = 5002 + reqInfo.key % 23;
    // uint16_t temp_port = 5002;
    // if(temp_port < 5002 || temp_port > 5024){
    //     printf("ERROR!!: dst_port = %d\n", temp_port);
    // }
    ...
    if(reqInfo.op_type == OP_GET || reqInfo.op_type == OP_GET_RESEND || reqInfo.op_type == OP_DEL){
        ...
        // message_header->clientID = htons(reqInfo.cid);

        // message_header->key[0] = reqInfo.key;
        unsigned char key_tmp[KEY_BYTES] = {0};
            ...
            for (int i = 0; i < lconf->n_rx_queue; i++) {
                ...
                for (int j = 0; j < nb_rx; j++) {
                    ...
                    // if(rcv_cid != LOCKID_NOTFIT){
                        ...
                        // printf("dst_port = %u, src_port = %u, rcv_mode = %u, rcv_opt = %u, rcv_txn_id = %u, rcv_cid = %u, rcv_padding = %u, rcv_lockId\n", ntohs(udp->dst_port), ntohs(udp->src_port), rcv_mode, rcv_opt, rcv_txn_id, rcv_cid, rcv_padding, rcv_lock_id);
                        ...
                        if(rcv_cid_idx >= 0 && rcv_cid_idx < MAX_CLIENT_IN_LCORE){   
                            ...
                            for(int k = 0; k < txn_arr[rcv_cid_idx].lock_num; k++){
                                ...
                                if(lockInfo.lock_id == rcv_lock_id){
                                    // printf("907 lockInfo.lock_id == rcv_lock_id\n");
                                    //statistics
                                    // printf("rcv lock_id:%d, txn_id:%d, cid:%d, op_type:%d, latency:%f\n", lock_id, txn_id, cid, message_header->op_type, latency);
...
static int32_t np_client_txrx_loop_kvs(uint32_t lcore_id, uint32_t client_id, uint16_t rx_queue_id, struct_zipfPara zipfPara, FILE* fout, FILE* fout_fpgaCheck) {
    ...
    uint32_t *timestamp_arr = (uint32_t *)malloc(sizeof(uint32_t *) * 1u * 100 * 1000 * 1000);
    uint32_t *req_arr = (uint32_t *)malloc(sizeof(uint32_t *) * 1u * 100 * 1000 * 1000);
    uint32_t *periodstart_arr = (uint32_t *)malloc(sizeof(uint32_t *) * 1u * 100 * 1000 * 1000);
    uint32_t *periodend_arr = (uint32_t *)malloc(sizeof(uint32_t *) * 1u * 10 * 1000 * 1000);
    double *frequency_arr = (double *)malloc(sizeof(double *) * 1u * 1000);
    if (timestamp_arr == NULL || req_arr == NULL) {
    ...
    char reqFile[128];
    sprintf(reqFile, "/home/zxy/dataset/Hare/cache-trace/cluster20/time_thres_5000/cleaned_data.tsv");
    FILE *file = fopen(reqFile, "r"); // 打开文件
    if (file == NULL) {
    ...
    while (fgets(buffer, sizeof(buffer), file) != NULL) {
        uint32_t timestamp, key, key_size, value_size, periodstart, periodend;
        // 使用 sscanf 解析每一行的数据
        if (sscanf(buffer, "%u\t%u\t%u\t%u\t%u\t%u", &timestamp, &key, &key_size, &value_size, &periodstart, &periodend) == 6) {
            timestamp_arr[line_count] = timestamp;
            req_arr[line_count] = key;
            periodstart_arr[line_count] = periodstart;
            periodend_arr[line_count] = periodend;
            line_count++;
        }
    }

    uint32_t duplicate_times = 100; //>=0  (500)
    uint32_t compress_ratio = 100; //>=1;
    uint32_t debug_end = 0xfffffff;
    printf("INFO: trace OK [thread %d]\n", lcore_id);
    // pthread_barrier_wait(&trace_init);
    //-------------------------------------
    //-------------------------------------

    uint32_t duplicate_count = 0;
    uint32_t reqIdx = 0;
    uint32_t reqIdxNext = 0;
    uint32_t reqIdxStart = 0;
    uint32_t second = 0;
    uint32_t second_count = 0;

    sprintf(reqFile, "/home/zxy/dataset/Hare/cache-trace/cluster20/time_thres_1000/timestamp_frequency.csv");
    file = fopen(reqFile, "r"); // 打开文件
    if (file == NULL) {
        perror("Unable to open file");
        return 1;
    }
    ...
    // 读取文件的每一行，跳过第一行（标题）
    fgets(buffer, sizeof(buffer), file); // 跳过标题行
    while (fgets(buffer, sizeof(buffer), file) != NULL && line_count < 1000) {
        uint32_t timestamp, freuency;
        // 使用 sscanf 解析每一行的数据
        if (sscanf(buffer, "%u\t%u", &timestamp, &freuency) == 2) {
            frequency_arr[line_count] = (double)freuency;
            if (line_count > 1) frequency_arr[line_count] = frequency_arr[line_count] / frequency_arr[1];
            line_count++;
        }
    }
    frequency_arr[0] /= frequency_arr[1];
    frequency_arr[1] /= frequency_arr[1];
    //-------------------------------------
    //-------------------------------------

    struct_kvReqInfo * reqInfo_arr = (struct_kvReqInfo *)malloc(sizeof(struct_kvReqInfo) * MAX_CLIENT_IN_LCORE);
    int * getRespFlag_arr = (int *)malloc(sizeof(int) * MAX_CLIENT_IN_LCORE);
    for(int i = 0; i < MAX_CLIENT_IN_LCORE; i++){
    ...
    float drop_rate = zipfPara.drop_rate;
    int drop_thres = (drop_rate == 0) ? 0 : 1 / drop_rate;
    int drop_count = 0;
    printf("drop_rate = %f, drop_thres = %d\n", drop_rate, drop_thres);
    //---------------------------------------------------
        ...
        for(int c = 0; c < loop; c++){ 
            for (int i = 0; i < lconf->n_rx_queue; i++) {
                ...
                for (int j = 0; j < nb_rx; j++) {
                    ...
                    unsigned char key_tmp[KEY_BYTES] = {0};
                    for(int i=0; i<KEY_BYTES; i++){
                        key_tmp[i] = message_header->key[KEY_BYTES - 1 - i];
                    }                    
                    uint32_t rcv_key = ((uint32_t)key_tmp[0]) | ((uint32_t)key_tmp[1]) << 8 | ((uint32_t)key_tmp[2]) << 16 | ((uint32_t)key_tmp[3]) << 24;      
                    uint16_t rcv_cid = CLIENT_ID_BIAS;//ntohs(message_header->clientID);
                                  
                    int rcv_cid_idx = rcv_cid - (client_id - 1) * MAX_CLIENT_IN_LCORE - CLIENT_ID_BIAS;
                    if(rcv_cid_idx >= 0 && rcv_cid_idx < MAX_CLIENT_IN_LCORE){
                        struct_kvReqInfo reqInfo = reqInfo_arr[rcv_cid_idx];
                        // printf("504: rcv_cid_idx = %d, rcv_key = %d, reqInfo.key=%d\n", rcv_cid_idx, rcv_key, reqInfo.key);
                        if(reqInfo.key == rcv_key){
                            if(getRespFlag_arr[rcv_cid_idx] == RESPFLAG_ACQUIRE_WAIT && rcv_opt == OP_GETSUCC){
                                getRespFlag_arr[rcv_cid_idx] = RESPFLAG_ACQUIRE_HAVEGOTTEN;
                            } 
                            // else if(getRespFlag_arr[rcv_cid_idx] == RESPFLAG_ACQUIRE_WAIT){
                            //     getRespFlag_arr[rcv_cid_idx] = RESPFLAG_ACQUIRE_INQUEUE;
                            // }
        ...
        if (pkts_send_ms < wpkts_send_limit_ms) {
            for(int c = 0; c < MAX_CLIENT_IN_LCORE; c++){
                ...
                if(stepFlag[c] == 0 && sendEnd_flag[c] == 0){
                    ...
                    reqIdx = reqIdxNext;
                    // if(timestamp_arr[reqIdx] == second){
                    if(!(timestamp_arr[reqIdx] < second * compress_ratio)){
                        if(reqIdx == line_count - 1){
                            reqIdxNext = 0;
                        }
                        else{
                            reqIdxNext = reqIdx + 1;
                        }
                        // if(timestamp_arr[reqIdxNext] > second){
                        if(timestamp_arr[reqIdxNext] == (second + 1) * compress_ratio){
                            if(duplicate_count == duplicate_times){
                                reqIdxNext = reqIdx;
                                continue;
                            }
                            else{
                                // reqIdxNext = periodstart_arr[reqIdx];
                                reqIdxNext = reqIdxStart;
                                duplicate_count++;
                            }
                        }
                    }
                    else{
                        // assert(timestamp_arr[reqIdx] < second);
                        assert(timestamp_arr[reqIdx] < second * compress_ratio);
                        if(periodend_arr[reqIdx] == line_count - 1 || periodend_arr[reqIdx] == debug_end){
                            reqIdxNext = 0;
                            reqIdxStart = 0;
                            second = 0;
                            // if(lcore_id == 1 && recir_flag == 0){
                            //     recir_flag = 1;
                            //     printf("INFO: Recirculation! [thread %d]\n", lcore_id);
                            // }
                        }
                        else{
                            reqIdxNext = periodend_arr[reqIdx] + 1;
                            reqIdxStart = periodend_arr[reqIdx] + 1;
                        }
                        // printf("INFO: timestamp_arr[reqIdx(%d)](%d) < second(%d), duplicate_count=%d, reqIdxNext=%d, [thread %d]\n", reqIdx, timestamp_arr[reqIdx], second, duplicate_count, reqIdxNext, lcore_id);
                        duplicate_count = 0;
                    }


                    struct_kvReqInfo reqInfo;  
                    reqInfo.key = req_arr[reqIdx]; //4500000
                    // reqInfo.key = 1;
                    reqInfo.op_type = OP_GET;//OP_PUT_SWITCHMISS;//OP_GET;
                    reqInfo.value_len = 128;

                    reqInfo.cid = cid[c];
                    reqInfo.resend_count = 0;
                    reqInfo.last_send_time = cur_tsc;
                    reqInfo.req_start_time = cur_tsc;
                    reqInfo_arr[c] = reqInfo;
                    getRespFlag_arr[c] = RESPFLAG_ACQUIRE_WAIT;
                    // printf("504: send_cid_idx = %d, send_key = %d\n", reqInfo.cid, reqInfo.key);
                    
                    //fprintf(fout, "613 step 0: send lock_id:%d, txn_id:%d, cid:%d\n", lockInfo.lock_id, txnInfo.txn_id, txnInfo.cid);
                    // if(drop_count == drop_thres - 1){
                    //     drop_count = 0;
                    // }
                    // else{
                    //     mbuf = rte_pktmbuf_alloc(pktmbuf_pool[rx_queue_id]); 
                    ...
                    mbuf = rte_pktmbuf_alloc(pktmbuf_pool[rx_queue_id]); 
                    generate_kv_request_pkt(rx_queue_id, header_template_local, lcore_id, mbuf, reqInfo, ip_src_pton, ip_dst_pton, 0);
                    enqueue_pkt_with_thres(lcore_id, mbuf, 1, 0); // enqueue_pkt_with_thres(lcore_id, mbuf, txnInfo.lock_num, 0);
                    ...
                    // printf("im here\n");
                    struct_kvReqInfo reqInfo = reqInfo_arr[c];
                    int resendFlag = 0;
                    int deadLockFlag = 0;
                    int allSuccFlag = 1; 
                    ...
                    if(cur_tsc > next_deadlock_check_tsc[c]){
                        deadLockFlag = 1;
                        tput_stat[lcore_id].req_timeout += 1;
                    }

                    if(deadLockFlag == 0){
                        if(getRespFlag_arr[c] == RESPFLAG_ACQUIRE_WAIT){
                            allSuccFlag = 0;
                            if(unlikely(cur_tsc >= resend_check_interval_us + reqInfo.last_send_time && cur_tsc < next_deadlock_check_tsc[c] - padding_us)){
                            // if(unlikely(cur_tsc < next_deadlock_check_tsc[c] - padding_us)){
                                resendFlag = 1;
                                reqInfo.op_type = OP_GET_RESEND;//RE_GRANT_LOCK;

                                // printf("504: re_send_cid_idx = %d, re_send_key = %d\n", reqInfo.cid, reqInfo.key);
                                // if(drop_count == drop_thres - 1){
                                //     drop_count = 0;
                                // }
                                // else{
                                ...
                                reqInfo_arr[c] = reqInfo;
                                tput_stat[lcore_id].tx += 1;
                                tput_stat[lcore_id].tx_get_re4wait += 1;
                            }
                        }
                        // if(getRespFlag_arr[c] == RESPFLAG_ACQUIRE_INQUEUE){
                        //     allSuccFlag = 0;
                        //     // if(unlikely(cur_tsc >= resend_check_interval_us + reqInfo.last_send_time && cur_tsc < next_deadlock_check_tsc[c] - padding_us)){
                        //     if(unlikely(cur_tsc < next_deadlock_check_tsc[c] - padding_us)){
                        //         resendFlag = 1;
                    ...
                    if(deadLockFlag == 1 || allSuccFlag == 1){
                        ...
                        // if(cur_tsc > responseTime_upd_tsc){
                        //     printf("cur_tsc > responseTime_upd_tsc!\n");
                        // }
                        // if(getRespFlag_arr[c] == RESPFLAG_ACQUIRE_HAVEGOTTEN){
                        //     printf("succ! responseTime_count = %d, responseTime_upd_times = %d\n", responseTime_count, responseTime_upd_times);
                        // }
                        // else{
                        //     printf("wait or fail!\n");
                        // }
                        ...
                        // if(getRespFlag_arr[c] == RESPFLAG_ACQUIRE_HAVEGOTTEN){
                            getRespFlag_arr[c] = RESPFLAG_IDLE;

                            if(lcore_id == 1){
                                // printf("im here 0: cur_tsc = %lu, responseTime_upd_tsc = %lu!\n", cur_tsc, responseTime_upd_tsc);
                                if(responseTime_count < SAMPLE_PER_CHECK && cur_tsc > responseTime_upd_tsc){
                                    // printf("im here 1!\n");
                                    responseTime[responseTime_upd_times][responseTime_count] = cur_tsc - reqInfo.req_start_time;
                                    // printf("responseTime[%d][0] = %f\n", responseTime_count, (cur_tsc - reqInfo.req_start_time) / 1000000);
                                    responseTime_count++;
                                }
                            }
...
static int32_t np_client_statistic_loop(uint32_t lcore_id, uint32_t client_id, uint16_t rx_queue_id) {
    ...
    while (1) { 
        ...
        if(unlikely(cur_tsc > check_end_tsc)){
            printf("im here 0!\n");
            break;
        }
    ...
    //-----------------------------------------------------------begin-----------------------------------------------------------
        ...
        if ((lcore_id == 0) && (update_tsc > 0)) {        
            if (unlikely(cur_tsc > check_start_tsc && cur_tsc < check_end_tsc && cur_tsc > next_update_tsc)) {
                ...
                responseTime_upd_tsc = cur_tsc;
                responseTime_upd_times++;
                responseTime_count = 0;
...
#elif APP_KVS
    while ((opt = getopt(argc, argv, "n:a:r:s:h:z:w:e:d:")) != -1) {
        switch (opt) {
        case 'n':
            gb_zipfPara.n = atoi(optarg);
            break;
        ...
        case 'e':
            ...
            break;
        case 'd':
            gb_zipfPara.drop_rate = atoi(optarg) * 1.0 / 100;
            printf("gb_zipfPara.drop_rate = %f\n", gb_zipfPara.drop_rate);
            break;            
        default:
            nc_parse_args_help();
            return -1;
