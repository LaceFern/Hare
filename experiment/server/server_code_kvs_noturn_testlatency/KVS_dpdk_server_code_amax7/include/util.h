...
static void print_per_core_throughput(void) {
    ...
    for (j = 0; j < n_lcores; j++) {
        ...
        //     tput_stat[i].rx_succ - tput_stat[i].last_rx_succ
        //     );
        printf("rx_normal[%d]: %.2fMrps, tx_normal[%d]: %.2fMrps\n", i, 1.0 * (tput_stat[i].rx - tput_stat[i].last_rx) / 1000000, 
    ...
    printf("rx: %.2fMrps, tx_normal: %.2fMrps\n", rx_count / 1000000, tx_normal / 1000000);
}
