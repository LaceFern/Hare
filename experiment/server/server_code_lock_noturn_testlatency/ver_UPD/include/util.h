...
#define SHARED_LOCK             0x02
#define EXCLUSIVE_LOCK          0x01

#define MAX_TXN_NUM             1000000

#define MAX_LOCK_NUM            10010000//134217730//10010000//7010000
...
#define IP_SRC                  "10.0.0.7"
#define CLIENT_SLAVE_IP         "10.1.0.13"// no use
#define IP_DST                  "10.0.0.1"
...
// //对heap数组在[low, high]范围内向下调�?
// void downAdjust(heapNode heap[], int low, int high){
//     int i = low, j = i * 2;
//     while(j <= high){
//         if(j+1 <= high && heap[j+1].lockCounter < heap[j].lockCounter){ //TOPKCOLD的话小于号需要改成大于号
//             j = j + 1;
//         }
//         if(heap[j].lockCounter < heap[i].lockCounter){  //TOPKCOLD的话需要改成大于号
...
// static void init_header_template_local_ctrl(uint8_t header_template_local[HEADER_TEMPLATE_SIZE_CTRL]) {
...
//         //.addr_bytes = {0x98, 0x03, 0x9b, 0xc7, 0xc0, 0xa8}}; //amax 4
//         //.addr_bytes = {0x98, 0x03, 0x9b, 0xc7, 0xc8, 0x18}}; //amax 3
//         // .addr_bytes = {0x98, 0x03, 0x9b, 0xca, 0x40, 0x08}};
//         //.addr_bytes = {0x0c, 0x42, 0xa1, 0x2b, 0x0d, 0x70}}; //amax8
//     struct rte_ether_addr dst_addr = {
//         //.addr_bytes = {0x04, 0x3f, 0x72, 0xde, 0xba, 0x44}}; //amax 8 
//         // .addr_bytes = {0x98, 0x03, 0x9b, 0xc7, 0xc7, 0xfc}}; //amax 6 �?前CPU控制�?6上面
//         // .addr_bytes = {0x98, 0x03, 0x9b, 0xca, 0x40, 0x18}}; //amax 7
//         .addr_bytes = {0x98, 0x03, 0x9b, 0xc7, 0xc8, 0x18}}; //amax 3
//         // .addr_bytes = {0x98, 0x03, 0x9b, 0xca, 0x48, 0x38}}; //amax 2
//         //.addr_bytes = {0x00, 0x0a, 0x35, 0x02, 0x9d, 0xe8}};    //MAC addr of FPGA network stack1
//         //.addr_bytes = {0x00, 0x0A, 0x35, 0x02, 0x9D, 0xE9}}; //fpga mac 1 
...
static void nc_init(void) {
    ...
    for (portid_offset = 0; portid_offset < n_enabled_ports; portid_offset++) {
        ...
        for(i = 1; i < rx_queues_per_port; i++){
            ...
            struct rte_flow * flow = generate_udp_flow(portid, i + CLIENT_PORT, i / NC_RX_QUEUE_PER_LCORE, &error); // udpport,queue,error
            if (!flow) {
