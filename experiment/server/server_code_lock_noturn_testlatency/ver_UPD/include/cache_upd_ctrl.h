...
uint32_t ip_client_pton_arr[NUM_CLIENT] = {};
#define OP_USERUPD 0x00
...
uint16_t ether_type_switch_userupd = 0x2335;
...
typedef struct  CtrlHeader_switch_userupd_ {
    uint8_t         opType;
    uint32_t        objIdx;
    uint32_t        data[HOTOBJ_IN_ONEPKT];
    uint8_t         type[HOTOBJ_IN_ONEPKT];
    // uint8_t      ack;
    // uint8_t      padding[12];
} __attribute__((__packed__)) CtrlHeader_switch_userupd;
...
static void forService_generate_response_pkt(uint16_t rx_queue_id, uint8_t header_template[HEADER_TEMPLATE_SIZE], uint32_t lcore_id, struct rte_mbuf *mbuf, \
            uint8_t opType, uint8_t lockMode, uint16_t clientID, uint32_t lockID, uint32_t txnID, uint16_t dst_port, uint16_t amax_idx) {
    ...
    //DEBUG!!
    char* ip_src = IP_LOCAL;
    char* ip_dst = ip_client_arr[amax_idx];
    struct rte_ether_addr mac_src = {.addr_bytes = MAC_LOCAL};
    struct rte_ether_addr mac_dst = mac_client_arr[amax_idx];
    ...
    udp->src_port = htons(0);//htons(port_write + (uint32_t)(txnInfo.txn_id) % 128);
...
volatile uint32_t backendNode_stats_stage = STATS_LOCK_STAT;//STATS_WAIT;//STATS_LOCK_STAT;
...
uint32_t coldID[TOPK] = {0};
uint32_t coldIdx[TOPK] = {0};
...
static void forService_generate_userupd_pkt(uint16_t rx_queue_id, uint8_t header_template[HEADER_TEMPLATE_SIZE_CTRL], uint32_t lcore_id, struct rte_mbuf *mbuf, \
            uint16_t ether_type, uint8_t opType, uint32_t index) {
    struct lcore_configuration *lconf = &lcore_conf[lcore_id];
    assert(mbuf != NULL);
    ...
    mbuf->data_len += sizeof(CtrlHeader_switch_userupd);
    mbuf->pkt_len += sizeof(CtrlHeader_switch_userupd);
    CtrlHeader_switch_userupd* message_header = (CtrlHeader_switch_userupd*) ((uint8_t*)eth + HEADER_TEMPLATE_SIZE_CTRL);
    message_header->opType = opType;
    message_header->objIdx = htonl(index);
    for(int i = 0; i < 8; i++){
        message_header->data[i] = 0;
        message_header->type[i] = 0;
    }
}
...
//--------------------------------------------------------
static int32_t np_client_statistic_loop(uint32_t lcore_id, uint16_t rx_queue_id, struct rte_ether_addr src_addr, int ifvalue, struct rte_mempool * pktmbuf_pool_ctrl, int statistic_lcore_num, queue_list *lockqueues) {
...
//-----------------------------------------------------------process for ctrl-----------------------------------------------------------
        switch(backendNode_stats_stage){
                            ...
                            if(ntohs(eth->ether_type) == ether_type_fpga){
                                ...
                                if(message_header->opType == OP_LOCK_HOT_QUEUE){
                                    ...
                                    for(int k = 0; k < bound; k++){
                                        hotID[hotCount] = ntohl(message_header->obj[k]);
                                        coldIdx[hotCount] = ntohl(message_header->coldIdx[k]);
                                        coldID[hotCount] = ntohl(message_header->coldObj[k]);
                                        hotCount++;
                                    }
                                    if(hotNum == 0){
                                        backendNode_stats_stage = STATS_GET_HOT;
            ...
            case STATS_WAIT: { 
                    ...
                    //     //printf("826:hotID = %d, coldID = %d, coldIdx = %d\n", hotID[b], coldID[b], coldIdx[b]);
                    ...
                    //     // ȥ������������Ѷ�����Ϣ��������������           
                    //     mbuf = rte_pktmbuf_alloc(pktmbuf_pool_ctrl);
                    //     forService_generate_userupd_pkt(rx_queue_id, header_template_local_ctrl, lcore_id, mbuf, ether_type_switch_userupd, OP_USERUPD, coldIdx[b]);
                    //     enqueue_pkt_with_thres(lcore_id, mbuf, 1, 0);

                    //     uint32_t coming_flag = 0;
                    //     while(coming_flag == 0){
                    //         for (int i = 0; i < lconf->n_rx_queue; i++) {
                    //             uint32_t nb_rx = rte_eth_rx_burst(lconf->port, lconf->rx_queue_list[i], mbuf_burst, NC_MAX_BURST_SIZE);
                    //             for (int j = 0; j < nb_rx; j++) {
                    ...
                    //                 struct rte_ether_hdr* eth = rte_pktmbuf_mtod(mbuf_rcv, struct rte_ether_hdr *);
                    //                 //printf("826:stage = %d, ether_type = %d\n", backendNode_stats_stage, ntohs(eth->ether_type));
                    //                 if(ntohs(eth->ether_type) == ether_type_switch_userupd){
                    //                     CtrlHeader_switch_userupd* message_header = (CtrlHeader_switch_userupd*)((uint8_t*) eth + sizeof(CtrlHeader_switch_userupd));
                                        
                    //                     // //printf("826:checkpoint 0\n");
                    //                     for(int k = 0; k < 8; k++){
                    //                         if(message_header->type[k] != 0){
                    //                             // uint32_t tmp = ntohl(message_header->data[k]);
                    //                             // uint16_t dst_udp = tmp >> 16;
                    //                             // uint16_t cid = tmp & 0xffff;

                    //                             uint16_t tmp_dst_udp = message_header->data[k] >> 16;
                    //                             uint16_t tmp_cid = message_header->data[k] & 0xffff;
                    //                             uint16_t dst_udp = ntohs(tmp_dst_udp);
                    //                             uint16_t cid = ntohs(tmp_cid);

                    //                             uint16_t amax_idx = dst_udp / 8192 + 1 - 1;
                    //                             uint8_t mode = message_header->type[k];
                    //                             uint32_t lock_id = coldID[b];
                    //                             uint32_t txn_id = cid;
                    //                             uint64_t timestamp = 0;
                    //                             //printf("826:checkpoint 1: amax_idx = %d, dst_udp = %d, data = %x\n", amax_idx, dst_udp, message_header->data[k]);
                                                
                    //                             mbuf = rte_pktmbuf_alloc(pktmbuf_pool_ctrl);
                    //                             forService_generate_response_pkt(rx_queue_id, header_template_local, lcore_id, mbuf, OP_GRANT_SUCCESS, mode, cid, lock_id, txn_id, dst_udp, amax_idx);
                    //                             enqueue_pkt_with_thres(lcore_id, mbuf, 1, 0);
                    //                             // //printf("826:checkpoint 2\n");
                    //                         }
                    //                         else{
                    //                             if(message_header->type[k] != 0){
                    //                                 //printf("826:checkpoint 2: type = %d\n", message_header->type[k]);
                    //                             }
                    //                         }
                    //                     }
                    //                     coming_flag = 1;
                    ...
                    //     //�ͷŵ�ǰ���У�hotID[b]���������
                    //     uint32_t lock_id = hotID[b];
                    //     queue_node *ptr = (&(lockqueues[lock_id]))->head;
                    //     while (ptr != NULL) {
                    //         uint16_t dst_udp = ptr->srcPort;
                    //         uint16_t amax_idx = dst_udp / 8192 + 1 - 1;
                    //         mbuf = rte_pktmbuf_alloc(pktmbuf_pool);
                    //         forService_generate_response_pkt(rx_queue_id, header_template_local, lcore_id, mbuf, OP_GRANT_SUCCESS, ptr->mode, ptr->client_id, lock_id, ptr->txnID, dst_udp, amax_idx);
                    //         enqueue_pkt_with_thres(lcore_id, mbuf, 1, 0);
                    //         ptr = ptr->next;
                    //     }
                    // }
                        ...
                        for (int j = 0; j < nb_rx; j++) {
                            ...
                            if(ntohs(eth->ether_type) == ether_type_fpga){
                                ...
                                if(message_header->opType == OP_HOT_REPORT_FPGA){    
                                    ...
                                    backendNode_stats_stage = STATS_REPLACE;    
            ...
            case STATS_REPLACE: {
                    ...
                    for(int b = 0; b < bound; b++){
                        ...
                        //                 // //printf("826:checkpoint 0\n");
                        //                 for(int k = 0; k < 8; k++){
                        //                     if(message_header->type[k] == 1 || message_header->type[k] == 2){
                        //                         uint32_t tmp = ntohl(message_header->data[k]);
                        //                         uint16_t dst_udp = (tmp & 0xffff0000) >> 16;
                        //                         uint16_t cid = tmp & 0xffff;
                        ...
                        //                         uint32_t txn_id = cid;
                        //                         uint64_t timestamp = 0;
                        //                         //printf("826:checkpoint 1: amax_idx = %d, dst_udp = %d, cid = %d, mode = %d, data = %x\n", amax_idx, dst_udp, cid, mode, tmp);
                        ...
                        while (ptr != NULL) {
                            uint16_t dst_udp = ptr->srcPort;
                            uint16_t amax_idx = dst_udp / 8192 + 1 - 1;
                            //printf("826:checkpoint 1: amax_idx = %d\n", amax_idx);
                            if(amax_idx < NUM_CLIENT){
                                mbuf = rte_pktmbuf_alloc(pktmbuf_pool_ctrl);
                                forService_generate_response_pkt(rx_queue_id, header_template_local, lcore_id, mbuf, OP_GRANT_SUCCESS, ptr->mode, ptr->client_id, lock_id, ptr->txnID, dst_udp, amax_idx);
                                enqueue_pkt_with_thres(lcore_id, mbuf, 1, 0);
                            }
...
static void forCtrlNode_generate_ctrl_pkt_fetchCold(uint16_t rx_queue_id, uint8_t header_template[], uint32_t lcore_id, struct rte_mbuf *mbuf, \
    ...
    rte_ether_addr_copy(&dst_addr, &eth->d_addr);
    //printf("826:send: ether_src = %d, ether_dst  = %d, ether_type = %d\n", eth->s_addr, eth->d_addr, ntohs(eth->ether_type));
