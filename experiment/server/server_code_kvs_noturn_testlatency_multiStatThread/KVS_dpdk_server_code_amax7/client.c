...
#include "./include/cache_upd_ctrl_mst.h"
...
static void generate_write_request_pkt_server(uint16_t rx_queue_id, uint8_t header_template[HEADER_TEMPLATE_SIZE], uint32_t lcore_id, struct rte_mbuf *mbuf, \
            uint8_t opType, char key[], uint8_t valueLen, char value[], uint32_t ip_src_addr, uint32_t ip_dst_addr, uint16_t dst_port, struct rte_ether_hdr* eth_client) {
...
static int32_t np_client_txrx_loop(uint32_t lcore_id, uint32_t client_id, uint16_t rx_queue_id) {
    ...
    uint64_t begin_test_tsc = 0;
    uint64_t end_test_tsc = 0;
        ...
        for (int i = 0; i < lconf->n_rx_queue; i++) {
            ...
            for (int j = 0; j < nb_rx; j++) {
                ...
                if (pkts_send_ms < wpkts_send_limit_ms){
                    ...
                    // service_statistic_update((*mapping_func)(lock_id), rx_queue_id - 1);
                    int ret = enqueue(rx_queue_id - 1, (*mapping_func)(lock_id));
                    while(ret == -1){
                        ret = enqueue(rx_queue_id - 1, (*mapping_func)(lock_id));
                    }
                    ...
                    if (opt == OP_GET || opt == OP_GET_RESEND) {  
                        ...
                        if(0){ // if(valueLenTable[lock_id] == 0){
                            mbuf = rte_pktmbuf_alloc(pktmbuf_pool);
                            generate_write_request_pkt_server(rx_queue_id, header_template_local, lcore_id, mbuf, OP_GETFAIL, key, 0, value, dstIP, srcIP, ntohs(udp->src_port), eth);
                            enqueue_pkt_with_thres(lcore_id, mbuf, NC_MAX_BURST_SIZE, 0);     
                            tput_stat[lcore_id].tx_resend += 1;                        
                        }
                        else{
                            mbuf = rte_pktmbuf_alloc(pktmbuf_pool);
                            generate_write_request_pkt_server(rx_queue_id, header_template_local, lcore_id, mbuf, OP_GETSUCC, key, valueLenTable[lock_id], valueTable[lock_id], dstIP, srcIP, ntohs(udp->src_port), eth);
                            enqueue_pkt_with_thres(lcore_id, mbuf, NC_MAX_BURST_SIZE, 0);                        
                    ...
                    //KVS OP_DEL           
                        ...
                        if(service_object_state_get(lock_id) == 1 || valueLenTable[lock_id] == 0){
                            mbuf = rte_pktmbuf_alloc(pktmbuf_pool);
                            generate_write_request_pkt_server(rx_queue_id, header_template_local, lcore_id, mbuf, OP_DELFAIL, key, 0, value, dstIP, srcIP, ntohs(udp->src_port), eth);
                            enqueue_pkt_with_thres(lcore_id, mbuf, NC_MAX_BURST_SIZE, 0);     
                            tput_stat[lcore_id].tx_resend += 1;                            
                        }  
                        else{
                            mbuf = rte_pktmbuf_alloc(pktmbuf_pool);
                            generate_write_request_pkt_server(rx_queue_id, header_template_local, lcore_id, mbuf, OP_DELSUCC, key, 0, value, dstIP, srcIP, ntohs(udp->src_port), eth);
                            enqueue_pkt_with_thres(lcore_id, mbuf, NC_MAX_BURST_SIZE, 0);   
                    ...
                    //KVS OP_PUT
                        ...
                        if(service_object_state_get(lock_id) == 1 || valueLenTable[lock_id] != 0){
                            mbuf = rte_pktmbuf_alloc(pktmbuf_pool);
                            generate_write_request_pkt_server(rx_queue_id, header_template_local, lcore_id, mbuf, OP_PUTFAIL, key, 0, value, dstIP, srcIP, ntohs(udp->src_port), eth);
                            enqueue_pkt_with_thres(lcore_id, mbuf, NC_MAX_BURST_SIZE, 0);     
                            tput_stat[lcore_id].tx_resend += 1;                            
                        }         
                        else if(opt == OP_PUT_SWITCHHIT){
                            mbuf = rte_pktmbuf_alloc(pktmbuf_pool);
                            generate_write_request_pkt_server(rx_queue_id, header_template_local, lcore_id, mbuf, OP_PUTSUCC_SWITCHHIT, key, valueLen, value, dstIP, srcIP, ntohs(udp->src_port), eth);
                            enqueue_pkt_with_thres(lcore_id, mbuf, NC_MAX_BURST_SIZE, 0);     
                            ...
                            mbuf = rte_pktmbuf_alloc(pktmbuf_pool);
                            generate_write_request_pkt_server(rx_queue_id, header_template_local, lcore_id, mbuf, OP_PUTSUCC_SWITCHMISS, key, 0, value, dstIP, srcIP, ntohs(udp->src_port), eth);
                            enqueue_pkt_with_thres(lcore_id, mbuf, NC_MAX_BURST_SIZE, 0);     
                            valueLenTable[lock_id] = valueLen;
                            memcpy(valueTable[lock_id], value, VALUE_BYTES);
                            if(0){ // if(valueLenTable[lock_id] == 0){
            ...
            if(lcore_id == 1 && nb_rx != 0){
                end_test_tsc = rte_rdtsc();
                // printf("query processing time = %f\n", 1.0 * (end_test_tsc - begin_test_tsc) / (rte_get_tsc_hz() / 1000000) / nb_rx);
            }
...
static int32_t np_client_loop(__attribute__((unused)) void *arg) {
    ...
    if ((lcore_id < 1)) {
        ...
        consume(appThreadNum, lcore_id - 36);
    }
    return 0; 
}
