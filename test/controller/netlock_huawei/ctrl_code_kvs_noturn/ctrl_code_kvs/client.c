...
static void main_ctrl(uint32_t lcore_id, uint16_t rx_queue_id, struct_ctrlPara ctrlPara){
    ...
    uint64_t start_time_debug = 0;
    uint64_t end_time_debug = 0;
        ...
        switch(ctrlNode_stats_stage){
            case STATS_COLLECT: { 
                if(cur_tsc > update_tsc){
                    ...
                    //     end_time_debug = rte_rdtsc();
                    //     printf("time for one turn = %f ms\n", 1.0 * (end_time_debug - start_time_debug) / tsc_per_ms);
                    //     exit(0);
                    // }
                    // if(time_debug_flag == 0){
                    //     start_time_debug = rte_rdtsc();
                    //     time_debug_flag = 1;
                    // }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                //printf("211OP_GET_COLD_COUNTER_SWITCH time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                end_time_tmp = rte_rdtsc();//-------------------------------
                //printf("211OP_GET_HOT_COUNTER_FPGA time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
                ...
                end_time_tmp = rte_rdtsc();//-------------------------------
                //printf("211replacement policy analyze time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
...
//-------------------------------------------------------------------
            case STATS_UPDATE: {
                ...
                end_time_tmp = rte_rdtsc();//-------------------------------
                //printf("211OP_LOCK_HOT_QUEUE time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
                ...
                // printf("828:rcv loop ends\n");
                end_time_tmp = rte_rdtsc();//-------------------------------
                //printf("211QUEUE EMPTY time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
                ...
                // printf("828:rcv loop ends\n");
                end_time_tmp = rte_rdtsc();//-------------------------------
                //printf("211OP_HOT_REPORT time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
                ...
                // printf("828:rcv loop ends\n");
                end_time_tmp = rte_rdtsc();//-------------------------------
                //printf("211OP_REPLACE_SUCCESS time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
            ...
            case STATS_CLEAN: {
                ...
                // printf("828:rcv loop ends\n");
                end_time_tmp = rte_rdtsc();//-------------------------------
                //printf("211OP_CLN_HOT_COUNTER_FPGA time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
                ...
                enqueue_pkt_with_thres(lcore_id, mbuf, 1, 1);
                end_time_tmp = rte_rdtsc();//-------------------------------
                //printf("211OP_CLN_COLD_COUNTER_SWITCH time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
...
//                 }
                ...
                // printf("828:rcv loop ends\n");
                end_time_tmp = rte_rdtsc();//-------------------------------
                //printf("211OP_UNLOCK_STAT time = %f ms\n", 1.0 * (end_time_tmp - start_time_tmp) / tsc_per_ms);//-------------------------------
