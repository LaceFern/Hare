...
static void get_traces(uint32_t lcore_id, uint32_t **txn_id, uint32_t **action_type, uint32_t **target_lm_id,
                uint32_t **target_obj_idx, uint32_t **lock_type, uint32_t* len, char *filename, int line_c) {
    FILE *fin, *fp;
    ...
    // (*txn_id) = (uint32_t *) rte_malloc(NULL, sizeof(uint32_t) * (line_c + 1));
    // (*action_type) = (uint32_t *) rte_malloc(NULL, sizeof(uint32_t) * (line_c + 1));
    // (*target_lm_id) = (uint32_t *) rte_malloc(NULL, sizeof(uint32_t) * (line_c + 1));
    // (*target_obj_idx) = (uint32_t *) rte_malloc(NULL, sizeof(uint32_t) * (line_c + 1));
    // (*lock_type) = (uint32_t *) rte_malloc(NULL, sizeof(uint32_t) * (line_c + 1));
