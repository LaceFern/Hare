...
#define CLEAN_FLAG()                                          \
    Register<bit<1>, bit<1>>(1) clean_flag_reg;               \
    RegisterAction<                                               \
        bit<1>,                                                   \
        bit<1>,                                                   \
        bit<1>                                                    \
    >(clean_flag_reg) clean_flag_get = {                  \
        void apply(inout bit<1> register_data, out bit<1> result) \
        {                                                         \
            result = register_data;                               \
        }                                                         \
    ...
    RegisterAction<                                               \
        ...
        bit<1>,                                                   \
        bit<1>                                                    \
    >(clean_flag_reg) clean_flag_rst = {                  \
        void apply(inout bit<1> register_data)                    \
        {                                                         \
            register_data = 0;                                    \
        }                                                         \
    };                                                            \
    action get_clean_flag(out bit<1> flag)                    \
    {                                                             \
        flag = clean_flag_get.execute(0);                     \
    }                                                             \
    action set_clean_flag()                                   \
    {                                                             \
        clean_flag_set.execute(0);                            \
    }                                                             \
    action rst_clean_flag()                                   \
    {                                                             \
        clean_flag_rst.execute(0);                            \
    }

#define CLEAN_FLAG_GET(flag) \
    get_clean_flag(flag)

#define CLEAN_FLAG_SET() \
    set_clean_flag()

#define CLEAN_FLAG_RST() \
    rst_clean_flag()
